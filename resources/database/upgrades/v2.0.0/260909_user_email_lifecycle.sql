-- 260909 U2 用户账号生命周期：邮箱注册列 + 软删时间 + 注销邮箱墓碑表（doc 15 §2.2.4/§2.2.5/§2.2.6）
...
-- 注销邮箱墓碑：SHA256(email) 留 180 天，防重复注册滥用回查（doc 15 §2.2.5；
-- 同邮箱二次注销走 upsert 刷新 expire_time）。到期清理由保留期任务负责（U6）
