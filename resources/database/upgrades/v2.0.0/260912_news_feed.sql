-- 260912 U12-A 资讯流数据模型 + 种子（doc 19 §3/§2，wayfinder T4 单元 A1）
-- 资讯流与 RAG 证据面物理隔离：四表独立于知识库管线，检索链路不读；
-- 条目按 url_hash=sha256(url) 幂等去重；90 天保留清理由 NewsRetentionJob 负责（不进 U6 DataRetentionProperties）。
-- 来源终表见 doc 19 §2：微博/知乎 robots+反爬双否决不入库；campus-reports 停更（最新 2023-06）种子即 enabled=false。

-- 信源注册表：fetch_strategy 四型（SITEMAP/HTML_LIST/RSS/JSON_API）；
-- consecutive_failures 沿用 K2c 滞回范式（阈值 3 自动禁源）
...
-- LLM 新提案以 curated=false 入库不进目录，晨报抽样审后转正/合并/丢弃
...
-- ============ 种子：信源 7 行（doc 19 §2 终表） ============
-- campus-reports 停更（最新 2023-06）默认禁用；PRN 月更低频（周更补充源）；
-- YouTube RSS 生产香港服务器可达即接入（T1 遗留：本机 DNS 污染未实测端点）
