...
interface AgentChatState {
  ...
  // 游客配额超限（agent 链 guard 拒绝走 SSE error 事件→onError 分类 quota）——
...
export const useAgentChatStore = create<AgentChatState>((set, get) => {
  ...
  const runStream = async (params: {
    ...
    const handlers = {
      ...
      onError: (error: Error) => {
        if (get().streamingMessageId !== assistantId) return;
        // 游客配额超限（agent 链 guard 拒绝）单独升维为 quota 弹窗状态，
        // 不走下方通用失败 toast——与 workflow 链 ChatQuotaModal 同语义
        const quotaError = classifyChatError(error.message) === "quota" ? error.message : null;
        set((state) => ({
          ...STREAM_IDLE,
        ...
        toastErrorUnlessShown(error, "生成失败");
      }
    };
  ...
  return {
    ...
    renameSession: async (sessionId, title) => {
      ...
      try {
        ...
        toastErrorUnlessShown(error, "重命名失败");
      }
    },
