...
interface MessageListProps {
  ...
  // 会话消息加载失败文案与重试回调——空态优先于欢迎屏渲染行内错误
  loadError?: string | null;
  onRetry?: () => void;
}

export function MessageList({ messages, isLoading, isStreaming, sessionKey, loadError, onRetry }: MessageListProps) {
    ...
    if (loadError) {
      return (
        <div className="flex h-full items-center justify-center px-6">
          <div className="flex w-full max-w-sm flex-col items-center gap-4 rounded-2xl border border-black/5 bg-white px-8 py-10 text-center shadow-sm">
            <p className="text-sm leading-relaxed text-[#5F6368]">{loadError}</p>
            {onRetry ? (
