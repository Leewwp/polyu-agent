...
import { AlertTriangle, Brain, ChevronDown, Clock3, Hourglass } from "lucide-react";
...
/**
 * 结构化提示块：排队/在途互斥等以消息内提示呈现。
 * quota（配额用尽）升级为 ChatQuotaModal 全局弹窗，行内块让位不渲染。
 */
    ...
    <div className="group flex">
      <div className="min-w-0 flex-1 space-y-4">
        ...
        <div className="space-y-2">
          {isWaiting ? (
            <div className="ai-wait" aria-label={message.awaitingSignal ? (zh ? "排队等待中" : "Waiting in queue") : zh ? "思考中" : "Thinking"}>
              {message.awaitingSignal ? (
                <p className="text-xs text-[#999999]">
                  <span className="inline-flex items-center gap-1.5">
                    <Hourglass className="h-3.5 w-3.5 animate-pulse-soft" />
                    {zh ? "已提交，正在排队等待系统空闲，高峰期可能需要数十秒" : "Submitted and waiting for a free slot — this can take tens of seconds at peak times"}
                  </span>
                </p>
              ) : null}
