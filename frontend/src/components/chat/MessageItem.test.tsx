...
describe("MessageItem 结构化提示（超限非裸 toast）", () => {
  ...
  it("配额用尽不再渲染行内提示块（升级为 ChatQuotaModal 全局弹窗）", () => {
    mount(assistantMessage({ kind: "quota", text: "今日匿名试用次数已用完，注册登录后可继续提问" }));

    // 行内块整体让位（含原注册引导链接），quota 文案由 ChatPage 挂载的弹窗承载
    expect(screen.queryByText(/今日匿名试用次数已用完/)).toBeNull();
    expect(screen.queryByRole("link", { name: /注册登录，解锁完整使用/ })).toBeNull();
  });
