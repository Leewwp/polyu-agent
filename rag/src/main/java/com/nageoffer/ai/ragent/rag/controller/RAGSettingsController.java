...
public class RAGSettingsController {
    ...
    /**
     * 引擎档位迷你端点（2026-09-13 定稿）：只返 {type}，登录态即可读——
     * 本路径不在 SaTokenConfig.ADMIN_PATH_PATTERNS 的 /rag/settings 精确模式内（该模式
     * 不带通配，不匹配子路径），供前端 EngineGate 探测引擎；此前非 admin 用户经
     * admin-only 的 /rag/settings 探测必 403 被静默回退 workflow，v2 Agent 仅 admin 可达。
     */
    @GetMapping("/rag/settings/engine")
    public Result<SystemSettingsVO.EngineSettings> engine() {
        return Results.success(SystemSettingsVO.EngineSettings.builder()
                .type(orchestrationProperties.getMode().name().toLowerCase(Locale.ROOT))
                .build());
    }
