...
/**
 * 抓取编排服务（U12-A A3，doc 19 §4-2 抓取序列的 A3 段）
 *
 * <p>每源序列：策略分派 → 四型抓取器取原始条目 → 14 天回灌窗口过滤 →
 * url_hash 幂等去重 → 批上限 → 入库 published。失败滞回沿 K2c 判例：
 * 成功（哪怕 0 新条目）清零 consecutive_failures；失败 +1，≥3 自动置
 * enabled=false（需人工复归——结构改版调查后再开）。
 *
 * <p>14 天窗口（§13-2）：首次回灌防首页空窗，之后靠 url_hash 去重自然纯增量
 * （窗口外旧条目即使重新出现在列表页也不入库）；events 型活动开始时间可在未来，
 * 窗口只设下界不设上界。LLM 摘要/分类/topics 与热度模型归 A4/A5，本类不碰。
 */
@Slf4j
@Service
public class NewsFetchService {
     ...
     * 回灌窗口天数（§13-2 建议 14 天）
     */
    static final int BACKFILL_DAYS = 14;

    /**
     * 单源单轮入库上限（批上限防长事务，doc 19 §4-1）
     */
    static final int MAX_ITEMS_PER_SOURCE = 50;

    /**
     * 滞回阈值：连续失败 ≥3 自动禁源（K2c 判例）
     */
    ...
    public NewsFetchService(List<NewsSourceFetcher> fetchers,
        ...
        this(fetchers, sourceMapper, itemMapper, Date::new);
    }
    ...
    /**
     * 窗口过滤 + 去重 + 批上限 + 入库（status=published；A5 LLM 补双语与分类）
     */
    int persistNewItems(Long sourceId, List<RawNewsItem> items) {
        Date now = nowSupplier.get();
        Date windowFloor = new Date(now.getTime() - BACKFILL_DAYS * 24L * 3600 * 1000);
        ...
        for (RawNewsItem item : inWindow.values()) {
            if (inserted >= MAX_ITEMS_PER_SOURCE) {
                log.warn("[news] 单源单轮入库达上限 {} 条，其余留待下轮：源条目 {}", MAX_ITEMS_PER_SOURCE,
                        inWindow.size());
                break;
            }
    ...
    /**
     * 失败滞回 +1；连续 ≥3 自动禁源（K2c 判例：禁用是防持续打爆坏端点，复归=人工）
     */
