 ...
 * 建议初始 3 次/日；额度终值与启用由部署方确定（ragent.anonymous.*，默认关）。
 */
@Component
...
public class AnonymousTrialGuard {
    ...
    /**
     * 配额「日」界固定香港时区：生产容器若为 UTC，JVM 默认时区的配额日
     * 在 08:00 HKT 切换，HKT 07–09 窗口可连取两日额度；对照 NewsFetchJob.HKT_ZONE 先例
     */
    ...
    /**
     * 匿名每日试用次数；0/负数 = 不限制（额度终值由部署方确定，建议 3）
     */
    ...
    private void consume(String key) {
        String todayKey = key + ":" + LocalDate.now(HKT_ZONE);
        Long count = stringRedisTemplate.opsForValue().increment(todayKey);
        if (count != null && count == 1L) {
            stringRedisTemplate.expireAt(todayKey, Date.from(LocalDate.now(HKT_ZONE).plusDays(1)
                    .atStartOfDay(HKT_ZONE).toInstant()));
        }
    ...
    private long readUsedCount(String key) {
        String value = stringRedisTemplate.opsForValue().get(key + ":" + LocalDate.now(HKT_ZONE));
        if (value == null) {
            return 0;
        }
        try {
