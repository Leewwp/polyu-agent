...
import com.nageoffer.ai.ragent.user.security.SecurityCorsProperties;
...
public class RagentWebMvcConfiguration implements WebMvcConfigurer {
    ...
    public void addCorsMappings(CorsRegistry registry) {
        // 安全收紧（吸收审计意见）：allowedOriginPatterns("*") + allowCredentials(true) 全开改为
        // 锁单 origin 白名单（SecurityCorsProperties 单一来源，默认仅生产域名）；
        // 生产为同源部署，edge 另以 proxy_hide_header 收敛 CORS 响应头兜底
        registry.addMapping("/**")
                .allowedOrigins(securityCorsProperties.getAllowedOrigins().toArray(String[]::new))
